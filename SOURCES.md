...
| 2026-06-01 | **SDSS spectral classes** (Channel C, RQ3) — *pre-data amendment* | Gentile Fusillo 2021 `sdssspec.dat.gz`, **same pinned release** `J/MNRAS/508/3877` | `3016e1a6…` | `data/derived/sdss_spectral.parquet` (gitignored) — 41,820 spectra (32,169 unique WDs); 894 unique metal-polluted (Z-class) WDs in the P_WD>0.75 sample (deduplicated — the table has multiple spectra per object). Fetcher: `pipeline/fetch/05_sdss_spectral.py`; analysis: `pipeline/analysis/10_clean_zone.py`. A different table of an already-pinned catalogue — no new survey. |
