...
| 2026-06-06 | **deviation** | §3 (youth floor) | **Registered youth floor not applied in T0.** The registered §3 activity-based youth-proxy floor (exclude gyro age < 1 Gyr, log L_X/L_bol > −4, or no-rotation + youngest variability decile) was **not** implemented in the first tier: the manifest builder applied only the Teff/logg/RUWE/parallax/CMD cuts, and the rotation-period, X-ray (eROSITA/ROSAT), and Gaia-variability cross-matches the floor needs were not incorporated. T0 therefore searched the *full* main-sequence K-dwarf manifest. Surfaced by adversarial (Gemini) review of the T0 draft. | A genuine deviation from the registered plan, disclosed. Effect is bounded: the floor would remove only the youngest/most-active stars, which already self-weight toward zero completeness, so the null is unaffected and the limit is simply over the full main-sequence population (a larger set), not an old-only subset. To be implemented before the limit is stated as old-only; carried to the next tier. The paper (§1, §2) states the non-application explicitly. |
