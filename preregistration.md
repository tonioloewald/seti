...
**Calibration and significance — the large-scale-inference (GWAS) toolkit.** Statistically, this is the **genome-wide association problem**: ~10⁵–10⁶ simultaneous tests in which essentially every object is null and the few real signals (if any) sit in the extreme tail. Rather than improvise criteria, we adopt that field's mature, pre-agreed machinery.

- **Empirical null** [Efron 2004]. The textbook χ² null is typically **too narrow**, because real photometric errors and the photosphere model are imperfect — trusting it would over-flag. We instead estimate the null's true width and location from the bulk of the population (where ~all objects are natural). Concretely we compute a genomic-control–style **inflation factor** `λ = median(observed A) / median(A under the theoretical null)` [Devlin & Roeder 1999]; `λ > 1` signals a miscalibrated (inflated) null, which we correct by rescaling to the empirical null *before* any object is called anomalous. The registered diagnostic is the **quantile–quantile plot** of observed vs. expected statistics: the bulk must track the null line (validating calibration), and candidates are the points departing in the extreme upper tail. *(This deliberately avoids inheriting a predefined notion of "normal"; a handful of genuine signals cannot bias a null defined by ~10⁵ ordinary objects.)*
- **Two registered significance criteria** (frozen procedures, computed numbers — §5.1), mirroring GWAS practice: **(i)** a strict family-wise **"anomaly-wide significance"** threshold — Bonferroni control of the family-wise error at α = 0.05 over the *effective number of independent tests* (computed from the data, exactly as the genome-wide 5×10⁻⁸ threshold is α = 0.05 over ~10⁶ independent variants) — defining the high-confidence subset; and **(ii)** a permissive **false-discovery-rate** criterion — Benjamini–Hochberg / Storey *q*-values [Benjamini & Hochberg 1995; Storey & Tibshirani 2003] at a frozen *q* — defining the broader candidate list we vet by hand (completeness-favoring, §1). The procedures and meta-parameters (α, *q*) are frozen here; the resulting score cutoffs are computed at analysis time.
- **Discovery, then replication.** As in GWAS, a flagged object is a **candidate, not a result**, until independently confirmed — here by archival cross-checks and dedicated follow-up (JWST/MIRI, future epochs; §5.4, §6). No object is presented as a confirmed anomaly on a single-pass statistic alone.
...
**Statistical methodology** (large-scale simultaneous inference, adopted from genomics):
